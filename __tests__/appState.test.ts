...
describe('app state invariants', () => {
  ...
  it('moves the default flag without leaving multiple defaults', () => {
    ...
    expect(state.servers[0]?.id).toBe(workId);
  });

  it('always keeps the default server at the top of the list', () => {
    let state = appReducer(initialSnapshot, {
      type: 'upsert-server',
      draft: {name: 'Alpha', url: 'https://alpha.example', isDefault: true},
    });
    state = appReducer(state, {
      type: 'upsert-server',
      draft: {name: 'Beta', url: 'https://beta.example', isDefault: false},
    });
    state = appReducer(state, {
      type: 'upsert-server',
      draft: {name: 'Gamma', url: 'https://gamma.example', isDefault: true},
    });

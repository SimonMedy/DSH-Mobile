...
export function normalizeServerUrl(input: string): string {
  ...
  const hostValue = isDefaultPort ? url.hostname : url.host;
  const normalizedPath = normalizePath(url.pathname);
  return `${url.protocol}//${hostValue}${normalizedPath === '/' ? '' : normalizedPath}`;
}
...
export function buildServerUrl(endpoint: EditableEndpoint): string {
  ...
  const base = `${endpoint.scheme}://${host}${portSuffix}`;
